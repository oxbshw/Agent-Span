//! Quora channel — reads public Quora questions/answers via Jina Reader.
//!
//! Quora has no public API, so reads and searches go through Jina Reader, which
//! renders pages to clean Markdown. Tier 0.
...
/// Quora channel (Jina Reader backed).
#[derive(Debug, Clone)]
pub struct QuoraChannel {
...
#[async_trait]
impl agentspan_core::channel::Channel for QuoraChannel {
    fn name(&self) -> &str {
        "quora"
    }

    fn description(&self) -> &str {
        "Read Quora questions and answers via Jina Reader (no API)"
    }
    ...
    async fn search(
        ...
        Ok(vec![SearchResult {
            title: format!("Quora results for \"{query}\""),
            url: search_url,
...
mod tests {
    ...
    #[test]
    fn can_handle_quora_urls() {
        let ch = QuoraChannel::new();
        assert!(ch.can_handle("https://www.quora.com/What-is-Rust"));
        assert!(!ch.can_handle("https://example.com"));
    }

    #[test]
    fn channel_is_tier_zero() {
        assert_eq!(QuoraChannel::new().tier(), Tier::Zero);
    }

    #[test]
    fn backend_is_jina() {
        let names: Vec<_> = QuoraChannel::new()
            .backends()
            .iter()
    ...
    async fn read_returns_page_text() {
        ...
        let ch = QuoraChannel::with_base_url(format!("{}/", server.uri()));
        let content = ch
            .read("https://www.quora.com/What-is-Rust", ReadOptions::default())
            .await
            .unwrap();
        assert!(content.body.contains("Rust is fast."));
    }
    ...
    async fn search_wraps_results_page() {
        let server = MockServer::start().await;
        Mock::given(method("GET"))
            .respond_with(ResponseTemplate::new(200).set_body_string("result one\nresult two"))
            .mount(&server)
            .await;
        ...
        assert!(results[0].snippet.contains("result one"));
