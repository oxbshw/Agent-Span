        ...
        match result {
            ...
            Ok(Err(e)) => {
                ...
                if kind == std::io::ErrorKind::NotFound {
                    // A bare name that isn't on PATH is genuinely missing. But if
                    // the program file exists yet still fails to spawn with
                    // NotFound, the file is present and broken — e.g. a script
                    // whose interpreter (a venv shebang) has been removed, which
                    // the kernel reports as ENOENT. Classify that as Broken.
                    if std::path::Path::new(program).is_file() {
                        // The file exists but its interpreter could not be found
                        // — almost always a stale virtual environment shebang.
                        ProbeResult {
                            status: ProbeStatus::Broken,
                            message: format!(
                                "{} has a stale virtual environment shebang (interpreter not found)",
                                program
                            ),
                            version: None,
                            hint: Some(
                                "Recreate the virtual environment or reinstall the tool"
                                    .to_string(),
                            ),
                        }
                    } else {
...
mod tests {
    ...
    /// Build a cross-platform command that exits with the given code. On Unix we
    /// use `sh -c`; on Windows `cmd /C` (there is no `false`/`exit` binary).
